...
:::warning[缺一不可]
这些缺了，Neovim 要么起不来，要么很多基础能力会直接失效。请在继续之前确认这些工具都已安装。
:::
...
:::danger[先备份，再动手]
如果你之前已经有自己的 Neovim 配置，**必须先备份**。LazyVim 这套配置会直接占用 `~/.config/nvim` 目录，不备份会导致旧配置丢失且无法回滚。
:::
...
:::info[耐心等待]
先别急着关掉，等它把该装的东西拉完。初次安装可能需要几分钟，取决于网络速度。
:::
...
:::tip[排查顺序]
如果装完之后"不太对劲"，大概率先从这几项查起。很多问题其实不用猜，健康检查会直接告诉你缺的是什么。
:::
