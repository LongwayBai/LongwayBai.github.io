...
先说结论：真正花时间的往往不是 `git clone`，而是把 Neovim、Node.js、搜索工具和语言环境补齐。配置本身装起来不难，难的是漏掉依赖之后，表面上能打开，实际很多功能用不起来。
...
这套配置通过 Mason 来管理 LSP（语言服务器）。装好基础环境后，如果你要写某种语言，去补对应的 Server 即可。
...
*图：Mason 的管理界面。在这里你可以看到哪些工具已装、哪些有更新。*
